...
{
    internal class DashboardNotificationHandler : IDisposable
    {
        ...
        private void UpdateOptionsInHomeAssistant()
        {
            if (!_notifications.Any())
            {
                ...
                _notificationCountEntity.SetValue(0);
                return;
            }
            ...
            var options = _notifications.Select(n =>
            {
                ...
                var (message, secondaryMessage) = ShortenMessages(
            ...
            _inputSelectEntity.CallService("set_options", new { options = options });
            _notificationCountEntity.SetValue(_notifications.Count);
        }

        // todo: test
        private (string, string?) ShortenMessages(string message, string? secondaryMessage, int amount)
        {
        ...
        {
            [JsonPropertyName("notificationDomain")]
